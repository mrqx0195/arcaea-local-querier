...
async function main(SQL) {
  ...
  bindOnClick('generate_image', event => {
    ...
    const pttInput = createElement('input', {
      type: 'text',
      placeholder: '你的ptt（请如实填入，用于计算r10）',
      style: {
        width: '100%',
        padding: '0.65rem 1rem',
        color: 'var(--text-color)',
    ...
    dialog.content(nameInput)
      ...
      .button('确定', async close => {
        ...
        const ptt = pttInput.value.trim();
        if (isEmpty(name)) alqSettings.remove('nickname');
        else alqSettings.set('nickname', name);
        await generate(name || void 0, songs, window.scores, ptt || void 0)
          .then(dataURL => {
